...
are detected and the amount of detected trascripts.
...
reference spots with coordinates and transcripts abundance.
Numer of cells in the spot depend on its size.

Technology examples of this family are: 10x Visium Spatial Gene Expression, Slide-seq, others.
...
This technology uses ad-hoc designed bacode-spots based chips where tissue slices 
can be placed on.
...
While we take a snapshop in full resolution of the on-chip places tissue slice, 
the unique barcoded-spots do cDNA amplification of the transcripts.

In such a way at the end we have an image reference of the experiment and a whole
transcriptome amplification per each spot.
...
* *spatialCoordsMtx*: returns the spatial coordinate (x_coord and y_coord) in matrix form.
* *imgData*: gets/sets the imgData
* *loadImg, unloadImg, addImg, removeImg*: imgData handling methods 
* *isInTissue*: returns a boolean of inTissue spots
...
We provided a main overview of Spatial Transcriptomics technologies, focusing on 
seqFISH for the molecule-based family and on 10x Genomics Visium Spatial Expression
for the spot-based ones.

We setup a class for spatial data analysis at single cell/spot level providing ad-hoc
...
If you're interested in a seqFISH dataset you can retrive it from the 
...
these datasets can be combined for multimodal single cell data integration methods
developments, testing and/or analysis.
More details about an investigation of these datasets can be found at this [link](https://drighelli.github.io/BIRSBIO2020.seqFISH.SpatialAnalysis/articles/seqFISH.html).
