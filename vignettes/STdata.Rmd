---
title: "3. The STdata package"
author:
  - name: "Lukas M. Weber"
    affiliation: "Johns Hopkins Bloomberg School of Public Health, Baltimore, USA"
    email: "lukas.weber@jhu.edu"
output: rmarkdown::html_vignette
vignette: >
    %\VignetteIndexEntry{3. The STdata package}
    %\VignetteEncoding{UTF-8}
    %\VignetteEngine{knitr::rmarkdown}
editor_options: 
  chunk_output_type: console
---

The [STdata package](https://github.com/lmweber/STdata) (under development) contains a collection of spatially resolved transcriptomics datasets, formatted into [SpatialExperiment](https://github.com/drighelli/SpatialExperiment) objects.

Example:


<!-- ```{r} -->
<!-- library("STdata") -->
<!-- library("ggspavis") -->
<!-- library("SpatialExperiment") -->
<!-- spe <- load_data("human_DLPFC") -->
<!-- spe -->
<!-- dim(spe) -->
<!-- assayNames(spe) -->
<!-- colData(spe) -->
<!-- rowData(spe) -->
<!-- spatialCoords(spe) -->
<!-- #plotSpots(spe) -->
<!-- #plotSpots(spe, discrete = "ground_truth", palette = "libd_layer_colors") -->
<!-- colData(spe) -->
<!-- colData(spe)$sum <- colSums(counts(spe)) -->
<!-- #plotSpots(spe, continuous = "sum", palette = "navy") -->
<!-- ``` -->

